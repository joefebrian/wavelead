...
import {
  Search, TrendingUp, BarChart3, Globe2, ShieldCheck, Sparkles, Users, ArrowRight,
} from 'lucide-react';
      ...
      <main>
              ...
              <div className="mt-8 flex flex-col sm:flex-row items-center justify-center gap-3">
                <Link href="/channels">
                  <Button size="lg" className="gap-2">
                    <Search className="h-4 w-4" /> Discover Channels
                  </Button>
                </Link>
                <Link href="/submit">
                  <Button size="lg" variant="outline" className="gap-2">
                    Submit your channel <ArrowRight className="h-4 w-4" />
                  </Button>
                </Link>
              </div>
        ...
        <section className="container py-16">
          <div className="text-center max-w-2xl mx-auto">
            ...
            <p className="mt-3 text-muted-foreground">
              WaveHub is a directory, an analytics platform and a promotion engine — in one place.
            </p>
          </div>
          ...
          <section className="container py-16">
            <div className="flex items-end justify-between mb-6">
              <div>
                ...
                <p className="text-muted-foreground text-sm mt-1">Explore what people follow on WaveHub.</p>
              </div>
            </div>
            <div className="grid gap-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
              {categories.map(cat => (
                <Link key={cat.id} href={`/category/${cat.slug}`}
                  className="wh-card p-4 flex items-center gap-3">
        ...
        <section className="border-y border-border/60 bg-secondary/40">
          <div className="container py-10 grid gap-6 md:grid-cols-3 text-sm">
            <TrustPoint icon={<ShieldCheck className="h-4 w-4" />} title="Owner-first & privacy safe"
              desc="We never store WhatsApp user identities. Channel data comes from owner submissions." />
            <TrustPoint icon={<Globe2 className="h-4 w-4" />} title="Built for a global audience"
              desc="Multi-country and multi-language architecture from day one." />
            <TrustPoint icon={<Users className="h-4 w-4" />} title="For creators & brands"
              desc="Free discovery for users. Freemium growth tools for channel owners." />
          </div>
        </section>
          ...
          <div className="rounded-2xl bg-foreground text-background p-10 md:p-14 relative overflow-hidden">
            ...
            <div className="relative max-w-xl">
              ...
              <p className="mt-3 text-background/80">
                Claim your profile, unlock analytics and start promoting. Free to submit.
              </p>
...
function TrustPoint({ icon, title, desc }) {
  return (
    <div className="flex gap-3">
      <div className="h-8 w-8 shrink-0 grid place-items-center rounded-md bg-primary/10 text-primary">{icon}</div>
      <div>
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="text-xs text-muted-foreground uppercase tracking-wider mt-0.5">
            {channel.country_code} · {channel.primary_language}
          </div>
          ...
          <div className="mt-3 flex items-center justify-between">
            <span className="text-xs text-muted-foreground">
              {Number(channel.follower_count || 0).toLocaleString()} followers
            </span>
